...
describe("projects and members integration", () => {
  let ready = false;
  ...
  it("returns 403 project_not_found for non-existent project push", async () => {
    if (!credentialsReady) {
      return;
    }
  ...
  it("creates project and lists members with pending invite", async () => {
    ...
    const projectName = `p4test${Date.now()}`;
    const create = await fetch(`${INTEGRATION_BASE_URL}/api/projects`, {
      method: "POST",
      headers: {
    ...
    expect(created.project.role).toBe("admin");

    const addInvite = await fetch(
      `${INTEGRATION_BASE_URL}/api/projects/${created.project.id}/members`,
      {
        method: "POST",
        headers: {
          ...csrfHeaders(CLIENT_IP),
  ...
  it("token scopes differ by project role", async () => {
    ...
    const projectName = `rbactest${Date.now()}`;
    const create = await fetch(`${INTEGRATION_BASE_URL}/api/projects`, {
      method: "POST",
      headers: {
        ...csrfHeaders(CLIENT_IP),
    ...
    const adminTokenResponse = await fetch(
      `${INTEGRATION_BASE_URL}/api/auth/token?service=registry&scope=repository:${projectName}/app:pull,push,delete`,
      {
  ...
  it("admin system role bypasses project membership for token", async () => {
    ...
    const projectName = `adminbypass${Date.now()}`;
    const sessionId = await loginSession();
    if (!sessionId) {
      return;
    }
    ...
    const tokenResponse = await fetch(
      `${INTEGRATION_BASE_URL}/api/auth/token?service=registry&scope=repository:${projectName}/repo:pull,push`,
      {
        headers: basicAuthHeader(
          INTEGRATION_ADMIN_EMAIL,
          INTEGRATION_ADMIN_PASSWORD,
